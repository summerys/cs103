...
int binsearch(int target, int * data, int start, int end)
{
  ...
  if (start <= end)
  {
    //Assigning middle point
    int middlePoint = (start + end) / 2;

    if (target == data[middlePoint])
    {
      return middlePoint;
    }
    else if (target < data[middlePoint])
    {
      return binsearch(target, data, start, middlePoint-1);
    }
    else 
    {
     return binsearch(target, data, middlePoint+1, end);
    }
  }//if
return -1;
} //int
...
void swap(int *data, int max, int len)
{
  int temp = 0;
